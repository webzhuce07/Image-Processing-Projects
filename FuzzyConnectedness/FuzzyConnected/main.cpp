...
const int WIDTH = 20;
const int HEIGHT = 20;
unsigned short TestingImage[400] = {
...
int main(int, char*[])
{
	...
	cv::Mat input(HEIGHT, WIDTH, CV_16UC1, &TestingImage);
	...
	for (i = 0; i < HEIGHT; i++) {
		for (j = 0; j < WIDTH; j++) {
			input.at<unsigned short>(i, j) = TestingImage[index];
			std::cout << std::setw(4) << TestingImage[index] << " ";
			index++;
		}
		std::cout << std::endl;
	}

	FuzzyConnectedness testFuzzy(input);
	/* execute the segmentation subroutine*/
	/* set input and the seed */
	testFuzzy.SetObjectSeed(cv::Point(5, 5));

	/* set the parameters */
	testFuzzy.SetParameters(270.0, 2500.0, 1.0, 1.0, 1.0);
	testFuzzy.SetThreshold(0.5);
	testFuzzy.MakeFuzzyScene();

	testFuzzy.UpdateThreshold(0.5);
	/* printout the segmentation result */
	std::cout << "Segmentation Result" << testFuzzy.GetThreshold() << std::endl;
	auto result = testFuzzy.GetSegmentObject();
	for (i = 0; i < HEIGHT; i++)
	{
		for (j = 0; j < WIDTH; j++)
			std::cout << result.at<unsigned short>(i, j) << " ";
		std::cout << std::endl;
	}


	testFuzzy.UpdateThreshold(0.8);
	std::cout << std::endl << "Update threshold" << testFuzzy.GetThreshold() <<std::endl;
	result = testFuzzy.GetSegmentObject();
	for (i = 0; i < HEIGHT; i++)
	{
		for (j = 0; j < WIDTH; j++)
			std::cout << result.at<unsigned short>(i, j) << " ";
