...
void OptimalPath( CvMat* I, vector<CvPoint> &Pixels, float x)
{	
	time6=0;
	...
	printf("计算时间：%d毫秒\n",time2-time1);
	time1 = clock();
	ShowMatrix1( FlagMat, "分割结果图", 500, 500, 1, false);
	time2 = clock();
	printf("绘图时间：%d毫秒\n",time2-time1);
	printf("找最大值时间：%d毫秒\n",time6);
...
void FindRoots( CvMat *I, vector<CvPoint> &Pixels, CvMat *FlagMat, vector<Coordinate> &Q)
{
	float l = 0.5;
	int x, y;
	...
	for(int si=0;si<Pixels.size();si++)
	{
		x = Pixels[si].x;
		y = Pixels[si].y;
		for(int i=-1;i<=1;i++)
	...
	for(int i=0;i<I->rows;i++)
		...
		{
			if(CV_MAT_ELEM( *FlagMat, float, i, j)>0.9999 && CV_MAT_ELEM( *FlagMat, float, i+1, j)>0.9999 && CV_MAT_ELEM( *FlagMat, float, i-1, j)>0.9999 && CV_MAT_ELEM( *FlagMat, float, i, j+1)>0.9999 && CV_MAT_ELEM( *FlagMat, float, i, j-1)>0.9999)
			{
...
{
	RootsMsg.s1 = RootsMsg.s1 + 0.000001;
	RootsMsg.s2 = RootsMsg.s2 * 0.7;
	...
	x2 = (CV_MAT_ELEM( *I, float, node1.y, node1.x)-CV_MAT_ELEM( *I, float, node2.y, node2.x)-RootsMsg.m2)/RootsMsg.s2;
