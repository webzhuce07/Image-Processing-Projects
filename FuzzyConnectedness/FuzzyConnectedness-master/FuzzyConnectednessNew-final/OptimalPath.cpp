...
CvMat* OptimalPath( CvMat* I, int rootX, int rootY, float x)
{
	...
	//if(false)
	{
		FindRoots( I, rootX, rootY, FlagMat, Q);
	}
	else
	{
		Coordinate RootNode;
	...
	printf("计算时间：%d毫秒\n",time2-time1);
	time1 = clock();
	ShowMatrix( FlagMat, "分割结果图", 500, 500, 2, false);
	time2 = clock();
	printf("绘图时间：%d毫秒\n",time2-time1);
	printf("找最大值时间：%d毫秒\n",time6);
	...
	/*for(int i=0;i<FlagMat->rows;i++)
		...
		{
			cout<<CV_MAT_ELEM( *FlagMat, float, i, j)<<endl;
		}*/
}
...
//	ShowMatrix( sobel, "梯度图", 780, 200, 2, false);
//	ShowMatrix( FlagMat, "标志矩阵图", 220, 500, 2, false);
	/*---------------------------------------------求梯度平均值*/
	float sobel_sum = 0.0;
	int sobel_num = 0;
	float sobel_mean;
